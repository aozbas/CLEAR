...
export default function App() {
  ...
  useEffect(() => {
    hosted_database.auth.getSession().then(({ data }) => {
      setSession(data.session);
      setLoading(false);
    });
    const { data: sub } = hosted_database.auth.onAuthStateChange((_event, s) => setSession(s));
...
function Home({ email }: { email: string }) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.homePadding}>
      ...
      <Pressable style={styles.secondary} onPress={() => hosted_database.auth.signOut()}>
