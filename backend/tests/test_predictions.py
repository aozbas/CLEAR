...
class PredictionRouteTests(unittest.IsolatedAsyncioTestCase):
    async def test_low_confidence_result_is_not_saved(self) -> None:
        ...
        with (
            patch.object(predictions, "hosted_database", fake_hosted_database),
            patch.object(
                predictions,
                "predict_lesion",
        ...
        fake_hosted_database.table.assert_not_called()
    ...
    async def test_saved_prediction_includes_model_version(self) -> None:
        ...
        fake_hosted_database = Mock()
        fake_hosted_database.table.return_value = insert_query
