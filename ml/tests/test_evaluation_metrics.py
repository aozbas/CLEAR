...
class EvaluationMetricsTests(unittest.TestCase):
    def test_confusion_matrix_uses_canonical_row_and_column_order(self) -> None:
        matrix = confusion_matrix(
            ["melanoma", "nevus", "squamous_cell_carcinoma"],
            ["nevus", "nevus", "melanoma"],
        )

        self.assertEqual(len(matrix), len(PAD_UFES_NATIVE_LABELS))
        self.assertEqual(len(matrix[0]), len(PAD_UFES_NATIVE_LABELS))
        self.assertEqual(matrix[2][3], 1)
        self.assertEqual(matrix[3][3], 1)
        self.assertEqual(matrix[4][2], 1)
    ...
    def test_macro_metrics_include_all_canonical_classes(self) -> None:
        ...
        self.assertAlmostEqual(summary["balanced_accuracy"], 1 / len(PAD_UFES_NATIVE_LABELS))
        self.assertAlmostEqual(summary["macro_precision"], (2 / 3) / len(PAD_UFES_NATIVE_LABELS))
        self.assertAlmostEqual(summary["macro_recall"], 1 / len(PAD_UFES_NATIVE_LABELS))
        self.assertAlmostEqual(summary["macro_f1"], 0.8 / len(PAD_UFES_NATIVE_LABELS))
    ...
    def test_prediction_distribution_counts_all_canonical_labels(self) -> None:
        ...
        self.assertEqual(set(distribution), set(PAD_UFES_NATIVE_LABELS))
        ...
        self.assertEqual(
            distribution["seborrheic_keratosis"],
